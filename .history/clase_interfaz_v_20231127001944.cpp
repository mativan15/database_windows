...
void InterfazV::menu(InterfazV vendedor1)	{
	bool repite = true;
	int opcion;
		...
		if (cin.fail() || (opcion != 1 && opcion != 2 && opcion != 3 && opcion != 4)) { //comprabacion de erroes
		    ...
		    switch (opcion){
					case 1:
						system("cls");
						//if
						exe_catalogo(vendedor1);
						repite = false;
						break;
					   
					case 2:
						system("cls");
						exe_pedidos(vendedor1);
						repite = false;
						break;
					case 3:
						system("cls");
						exe_canales(vendedor1);
						repite = false;
						break;
...
void InterfazV::exe_catalogo(InterfazV vendedor1) {
    Catalogo catalogo(Pedido1, lista_has, lista_viv, usuario1);
    catalogo.tab_catalogo(vendedor1);
    int op;
    bool retro = true;
    do {
        cout << "exe catalogo\x1B[34m" << "\n\t1. Regresar al Menu" << "\x1B[m" << "\n"
...
void InterfazV::exe_pedidos(InterfazV vendedor1) {
	Pedido1.tab_pedido(vendedor1);
	int op;
    bool retro = true;
    do {
...
void InterfazV::exe_canales(InterfazV vendedor1) {
    CanalesV canales1(Pedido1, lista_has, lista_viv, usuario1);
    canales1.tab_canales(vendedor1);
    int op;
    bool retro = true;
    do {
        cout << "exe canales\x1B[34m" << "\n\t1. Regresar al Menu" << "\x1B[m" << "\n"
             << "\x1B[34m" << "\n\t2. Regresar a Canales" << "\x1B[m" << "\n"
             << "\x1B[34m" << "\n\t3. Ir a Pedidos" << "\x1B[m" << "\n"
             << "\x1B[37m" << "\n\tEscoger Opcion: " << "\x1B[m";
        cin >> op;
        if (std::cin.fail() || !(op >= 1 && op <= 3)) {
