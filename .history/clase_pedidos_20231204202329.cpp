...
void Pedido::setCantpedido(int cant_p){
	if (cant_p >= 0) {
        cantpedido = cant_p;
    }
}
int Pedido::getCantpedido() const{
	return cantpedido;
}
...
void Pedido::print_pedido() {
    cout << "\n\tResumen Pedido: \n\n"
         ...
         << "Cantidad de Pedido: " << cantpedido << endl;   
}
void Pedido::set_lista_venta(Listas productos_anadidos){
    ...
    for (int i = 0; i < productos_anadidos.get_tamano_lista(); i++){
        ...
        cerr << canalesPtr->ventaPtr->getArray_productos()[i].get_cantidad() << endl;
        cerr << canalesPtr->ventaPtr->getArray_productos()[i].getNombreLibro() << endl;
