...
void pantalla_start(bool skip, Listas& lista_has, Listas& lista_viv, Usuario& usuario1){
	...
	int edad;
	int DNI;
	const char* ruta_data_array[] = {"../src/Project_0013/data_array.txt"};
	const char* ruta_data_ped[] = {"../src/Project_0013/data_ped.txt"};
	if (skip == false){
		system("cls");
...
void limpiar_archivo(const char *ruta_archivo){
	ofstream archivo(ruta_archivo, ios::trunc);
	if (!archivo) {
