...
class MiClase {
public:
    MiClase(int num, const std::string& texto) : numero(num), texto(texto) {}

    // Sobrecarga del operador de inserción <<
    friend std::ostream& operator<<(std::ostream& os, const MiClase& obj) {
        os << "Número: " << obj.numero << ", Texto: " << obj.texto;
        return os;
    }
...
int main() {
    MiClase miObjeto(42, "Hola, mundo!");
    
    // Imprimir todos los atributos de un objeto usando la sobrecarga del operador <<
    std::cout << miObjeto << std::endl;
